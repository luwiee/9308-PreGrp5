...
public class HelperFunctions {
    ...
    static int getUserMenuSingleDigitInput() {
        ...
        int input = Integer.parseInt(String.valueOf(my_scanner.next().charAt(0)));
        System.out.println();
        return input;
    }
    ...
    static int getIntInput() {
        Scanner my_scanner = new Scanner(System.in);
        return Integer.parseInt(my_scanner.next());
    }

    private static void swap_elements(int[] arr, int[] index_array, int j) {
        int temp;
        temp = arr[j - 1];
        arr[j - 1] = arr[j];
    ...
    static int[] bubbleSort(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 1; j < (n - i); j++) {
                if (arr[j - 1] < arr[j]) {
                    //swap elements
                    swap_elements(arr, index_array, j);
                }
            }
        }
    ...
    static void displayData() {
        ...
        // Distance
        System.out.format(format, "Total num of Doses", "Individuals with atlas One dose", "Fully Vaccinated", "Individuals with booster dose", "Total");
    ...
    // Function that gets an index and outputs the priority group assigned to that index as a string
    ...
    // Print Priority Group descending
    static void PrintDescendingPriorityGroupData() {
        ...
        // Sort the data using the default bubble sort
    ...
    static void PrintAscendingPriorityGroupData() {
        ...
        // Sort the data using the other bubble sort
